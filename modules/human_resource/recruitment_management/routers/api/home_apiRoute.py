...
async def apply(req: careers.Applicant, db: Session = Depends(get_db)):
    try:
        ...
        messageBody = f"""
            <h1 style="margin-bottom: 1px">Good day, {new_applicant.first_name}!</h1>
            <h3>This is from HoMIES - Recruitment Management System</h3>

            <p>We would like to inform you that your application for <b>{position.name}</b> is submitted successfully and it is now ready for evaluation. For some updates, please check your email regularly.</p>
            <p>Thank you!</p>
