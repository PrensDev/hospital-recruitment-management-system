...
from modules.human_resource.recruitment.schemas \
    import user_schemas as user, deptHead_schemas as deptHead
    ...
    try:
        if(authorized(user_data, AUTHORIZED_USER)):
            ...
            return db.query(Applicant).filter(or_(
                ...
                JobPost.requisition_id == ManpowerRequest.requisition_id
            ).join(Position).filter(
    ...
    try:
        if(authorized(user_data, AUTHORIZED_USER)):
            ...
            if not applicant:
                ...
                manpower_request = db.query(ManpowerRequest).filter(ManpowerRequest.requisition_id == job_post.requisition_id).first()
